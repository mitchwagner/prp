...
class NodeAndEdgeWithholdingFoldCreatorV2(fc.FoldCreatorV2):
    '''
    ...
    def create_negative_folds(self, dir_pos, undir_pos, dir_neg, undir_neg):
        ...
        for pair in pairs:
            dir_training_edges = pair[0]
            dir_test_edges = list(set(dir_pos) - set(dir_training_edges))

            undir_training_edges = pair[1]
            undir_test_edges = list(set(undir_pos) - set(undir_training_edges))
    ...
    def get_training_folds(self):
        ...
        folds = self.create_folds_filter()

        training_folds = []

        for i, fold in enumerate(folds):
            fold_name = self.get_fold_prefix(i)
            training_folds.append((fold[0], fold[2], fold_name))
    ...
    def get_test_folds(self):
        ...
        for i, fold in enumerate(folds):
            fold_name = self.get_fold_prefix(i)
            training_folds.append((fold[1], fold[3], fold_name))
