...
class RegLinkerRWERNoLoops(RankingAlgorithm):
    '''
    ...
    NOTE:
    No loops is a misnomer, shorthand for saying "no cycle 
    of length 2".
            ...
            with output_paths.open('r') as f1:
                for line in f1:
                    if line.startswith("#"):
                        ...
                        # NOW check that the paths have no two-edge loops

                        def is_loopy(path):
                            '''
                            If we have a path motif "aba", we want to 
                            eliminate this path
                            '''
                            ...
                            for i in range(len(path) - 2):
                                a = path[i]
                                b = path[i + 2] # Look two nodes ahead
    ...
    def get_name(self):
        return "RegLinker + RWER (No 2-Cycles)"


    def get_descriptive_name(self):
        return "RegLinker + RWER (No 2-Cycles), q=%s, rlcs=%s" % (self.q, self.rlc_abbr)
