...
#######################################################################
def main(args):
    usage = '''plot-precision-recall.py file1prefix file2prefix outname
'''
    if len(args) != 4:
        print usage
    ...
    for display in ['node','edge']:
        for negtype in ['none','adjacent']:
            ...
            if len(list1)==1: 
                ax.plot([r for p,r in list1], [p for p,r in list1],'sr',ms=6,label=file1.split('/')[-1])
            else: # plot line only.
                ax.plot([r for p,r in list1], [p for p,r in list1],'r',lw=2,label=file1.split('/')[-1])
            if len(list2)==1: 
                ax.plot([r for p,r in list2], [p for p,r in list2],'sb',ms=6,label=file2.split('/')[-1])
            else: # plot line only.
                ax.plot([r for p,r in list2], [p for p,r in list2],'b',lw=2,label=file2.split('/')[-1])
