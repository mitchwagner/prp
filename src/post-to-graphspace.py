...
GRAPHSERVER = 'http://holmes.cs.vt.edu/'
USERNAME = 'annaritz@vt.edu'
PASSWORD = 'platypus'
GROUP='2015PathwayReconstructions'
...
def constructGraph(receptors,tfs,prededges,increase,decrease,thres,netpath,kegg,prededgefile,undirected,nolabels):
    ...
    nodesize='auto' # text might be bigger than node otherwise
...
def main(args):
    ...
    client.add_graph(graph, opts.gsid, True)

    # Add graph to group
    groupcmd = 'GET -C %s:%s %sapi/users/%s/graphs/%s/share/%s' \
        % (USERNAME,PASSWORD,GRAPHSERVER,USERNAME,opts.gsid,GROUP)
    os.system(groupcmd)
    print 'DONE\n'
