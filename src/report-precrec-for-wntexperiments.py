...
## Now, determine network size by recall.
recalls = [0.2035,0.5]
#for experiment in ['wnt-all-receptors','netpath']:
for experiment in ['netpath']:
    if experiment == 'wnt-all-receptors':
    ...
    for negtype in negtypes:
        ...
        print '\n\nEXPERIMENT BY RECALLS: ',experiment,'NegType for last two columns:',negtype,'\n'
        ...
        lines = readColumns(infile,3,5,6)
        for r in recalls:
            index = min([i for i in range(len(lines)) if float(lines[i][2]) >r])-1
            numedges.append(index+1)
            nonepr.append((float(lines[index][1]),float(lines[index][2]),float(lines[index][0])))
        ...
        for i in range(len(recalls)):
            print '|PathLinker (recall<=%.3f)) | %d | %d | %.3f | %.3f | %.3f | %.3f|' % \
                (recalls[i],numedges[i],nonepr[i][2],nonepr[i][0],nonepr[i][1],adjpr[i][0],adjpr[i][1])
        ...
        numedges = []
        for r in recalls:
            index = max([i for i in range(len(lines)) if float(lines[i][2]) <r])
            numedges.append(index+1)
            nonepr.append((float(lines[index][1]),float(lines[index][2]),float(lines[index][0])))
        ...
        for r in recalls:
            ...
            adjpr.append((float(lines[index][1]),float(lines[index][2]),float(lines[index][0])))
        for i in range(len(recalls)):
            print '|PageRank (recall<=%.3f) | %d | %e | %.3f | %.3f | %.3f | %.3f|' % \
                (recalls[i],numedges[i],nonepr[i][2]*-1,nonepr[i][0],nonepr[i][1],adjpr[i][0],adjpr[i][1])
