'''
Runs an algorithm. Rather than using RWER to determine a direction for
undirected edges every single fold (like AlgorithmEvaluatorV2), this evaluator
runs a RWER to do this just once, using the union of all edges in the provided
pathways.

NOTE: This means that results will differ if you change the set of pathways
in consideration, which is less than desirable.
'''
...
class AlgorithmEvaluatorV3(Evaluator):
    '''
    ...
    def get_dir_map(self):
        # First check to see if the dir_map has been cached
        if hasattr(self, "dir_map"):
            print("Using cached dir_map")
            return self.dir_map

        print("Building dir_map")

        # Load the pathways
        all_netpath_edges = []

        print("Loading pathway edges")
        for pathway in self.pathway_collection.pathways:
            # Load the directed and undirected edges

            print("reading directed edges")
            dir_edges = iu.get_directed_pathway_edges(
                pathway.get_edges_file(), 
                self.interactome.direction_file)

            print("reading undirected edges")
            undir_edges = iu.get_undirected_pathway_edges(
                pathway.get_edges_file(), 
                self.interactome.direction_file)
            
            print("merging lists")
            all_netpath_edges = all_netpath_edges + dir_edges
            all_netpath_edges = all_netpath_edges + undir_edges
        
        print("taking set")
        all_netpath_edges = set(all_netpath_edges)
        ...
        # cache/return the dir_map that results from this procedure because
        # we will need it several times
        if not hasattr(self, "dir_map"):
    ...
    def get_training_folds(self):
        ...
        for fc in fold_creators:
            ...
            #folds.append((pos_training, neg_training))

            pos_list.append(pos_training)
            neg_list.append(neg_training)

            for x in pos_training:
                print(len(x))

            for x in neg_training:
                print(len(x))
