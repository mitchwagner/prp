...
class AlgorithmEvaluatorV2(Evaluator):
    '''
    ...
    def get_fold_creators(self):
        ...
        for pathway in self.pathway_collection.pathways:
            print("-------------------------------------------")
            print("Creating fold creator for", pathway)
            print("-------------------------------------------")
    ...
    def get_training_folds(self):
        ...
        for fc in fold_creators:
            print("-------------------------------------------")
            print("Creating training folds for", fc.pathway)
            print("-------------------------------------------")
    ...
    def run_reconstructions(self, output_dir=Path()):
        ...
        # We'll need the fold creators so that we can create the interactomes
        # for each fold
        fcs = self.get_fold_creators()
        ...
        for i, (pathway, training_fold) in enumerate(pairs):
            for j, fold in enumerate(training_fold):
                ...
                for algorithm in self.algorithms:
                    ...
                    # Get the appropriate fold creator from the creators
                    new_interactome_file = \
                        Path(alg_dir, "edge-filtered-interactome.txt")

                    fc = fcs[i]
                    fc.write_filtered_interactome(
                        new_interactome_file, j)
